...
class Dataset1(object):
    ...
    def load_train_file(self):# for single train
        filename = self.path + ".train.rating"
...
class Dataset(object):
    ...
    def load_test_file(self):
        filename = self.path + "test_data"#.test.rating
        ratingList = []
        with open(filename, "r") as f:
            line = f.readline()
            while line is not None and line != "":
    ...
    def load_negative_file(self):
        filename = self.path + "test_negative"
        negativeList = []
        with open(filename, "r") as f:
            line = f.readline()
            while line is not None and line != "":
    ...
    def load_client_train_date(self):#for FL train
        filename = self.path + "train_data"
        num_users, num_items = self.get_train_data_shape()
        ...
        with open(filename, "r") as f:
            for (usr, item) in mat.keys():
                ...
                line = f.readline()
                for t in range(4):#任意选择一个没有看的电影，作为negative项
                    nega_item = np.random.randint(num_items)
                    while (usr, nega_item) in mat.keys():
                        nega_item = np.random.randint(num_items)
                client_datas[usr][0].append(usr)
