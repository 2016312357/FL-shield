...
# 用梯度攻击！！！！！！！！！
random.seed(0)
# name = 'ml-10k-layer_0_1_2_3_epoch-500.npz'#-dp
name = 'ml-100k-layer_1_epoch-500-gc0.3.npz'
# name='ml-10k-layer_0_epoch-1000.npz'
# name='ml-100k-clean.npz'
# name = 'ml-100k-lap-5.npz'  # 'ml-10k-layer_0_1_2_3_epoch-1000.npz'
...
for weights, lab in zip(f['arr_0'], f['arr_1']):  # (38, 10, 8) (1, 380)
    for weight, la in zip(weights, lab):
        ...
        for layerid in [0, 1, 2, 3, 4, 6]:
            # print(weight[layerid].shape)

            attack_data[str(layerid)].append(
                np.expand_dims(np.expand_dims(weight[layerid], axis=0), axis=-1))  # .reshape((1,-1))
        # attack_dat=np.array(attack_data)
label = np.asarray(labels)
print(label.shape)
y_test = np.zeros([len(label), 21])
for i in range(len(y_test)):
    y_test[i, label[i]] = 1

model = {'0': None, '1': None, '2': None, '3': None, '4': None, '6': None}

from sklearn.model_selection import train_test_split

for layerid in [0, 1, 2, 3, 4, 6]:
    # print(attack_data[str(layerid)].shape)
    # train_x, attack_data[str(layerid)], train_y, test_y = train_test_split(attack_data[str(layerid)], label, test_size=0.3, random_state=0)
    # print(train_y)  # [1 8 0 ... 1 0 2]

    # for layerid in [0, 1, 2, 3, 4, 6]:
    # print(layerid)
    input_shape = attack_data[str(layerid)][0].shape[1:]
    # print(input_shape)
    model[str(layerid)] = model_attack2(input_shape=input_shape, num_classes=21)
    model[str(layerid)].compile(loss=keras.losses.categorical_crossentropy, optimizer=keras.optimizers.Adam(lr=0.001),
                                metrics=['accuracy'])

    model[str(layerid)].set_weights(
        np.load("./attack_checkpoint/model_attack_weights_layerid_{}.npz".format(layerid), allow_pickle=True)[
            'x'])  # attack_model_layerid_{}.npz 1; 1280,
    # model.summary()
print('loading done')
for i in range(len(label)):
    for layerid in [0, 1, 2, 3, 4, 6]:
        # [1,m,n,1]
        if layerid == 0:
            conf_score = model[str(layerid)].predict(attack_data[str(layerid)][i])
        else:
            conf_score += model[str(layerid)].predict(attack_data[str(layerid)][i])
            # print(conf_score)

    conf_score /= 6
    if np.argmax(conf_score) == label[i]:
        correct += 1

print(name, 'attack acc: ', correct, len(label), correct / len(label))
